...
systemctl --user disable --now nova7-audio-priority.service >/dev/null 2>&1 || true
...
rm -f "$HOME/.config/systemd/user/nova7-audio-priority.service"
...
rm -f "$HOME/.local/bin/nova7-audio-priority"
...
if [ -d "$HOME/.config/nova7-chatmix" ]; then
  read -rp "Remove audio-priority config (~/.config/nova7-chatmix)? [y/N] " answer
  if [[ "${answer,,}" == "y" ]]; then
    rm -rf "$HOME/.config/nova7-chatmix"
